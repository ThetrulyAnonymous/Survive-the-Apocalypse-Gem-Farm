...
-- --- 1. CONFIGURATION CHECK BEFORE LOADING UI ---
...
-- --- 2. SUPER FARM MODE ACTIVE (EAT ZERO MEMORY) ---
if Config_Data.OptimisedFarmActive then
    -- Turn off 3D rendering immediately
    pcall(function() RunService:Set3dRenderingEnabled(false) end)
    
    -- Setup fallback function so farm script doesn't error when logging
    getgenv().LogToTerminal = function(message, header)
        print(tostring(header) .. tostring(message))
    end

    -- Create ONLY a blank screen and one button
    local MinimalGui = Instance.new("ScreenGui")
    MinimalGui.Name = "SuperFarmMinimalPanel"
    ...
    BlackScreen.Parent = MinimalGui

    local DisableBtn = Instance.new("TextButton")
    DisableBtn.Size = UDim2.new(0, 400, 0, 50)
    DisableBtn.AnchorPoint = Vector2.new(0.5, 0.5)
    DisableBtn.Position = UDim2.new(0.5, 0, 0.5, 0)
    DisableBtn.BackgroundColor3 = Color3.fromRGB(20, 25, 35)
    DisableBtn.Font = Enum.Font.Code
    DisableBtn.TextSize = 22
    DisableBtn.TextColor3 = Color3.fromRGB(0, 255, 140)
    DisableBtn.Text = "[ Disable Optimised Farm Mode ]"
    DisableBtn.Parent = BlackScreen

    local stroke = Instance.new("UIStroke")
    stroke.Thickness = 1.2
    stroke.Color = Color3.fromRGB(0, 255, 140)
    stroke.Parent = DisableBtn

    DisableBtn.MouseButton1Click:Connect(function()
        Config_Data.OptimisedFarmActive = false
        writefile(CONFIG_FILE, HttpService:JSONEncode(Config_Data))
        
        -- Restore 3D rendering and tell player to rejoin/restart
        pcall(function() RunService:Set3dRenderingEnabled(true) end)
        DisableBtn.Text = "[ Enable Optimised Farm Mode Back ]"
...
StatusLabel.Text = "SCRIPT MADE BY NOTHIN [KEYLESS]"
...
OptimisedFarm.Text = "Note: Enabling this will delete UI for upcoming runs and you will just see black screen with button (Maximum performance)"
